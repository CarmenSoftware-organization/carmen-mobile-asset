...
const rows: AssetCountRow[] = [
  ...
  {
    asset: asset({ id: 'a3', code: 'AST003', name: 'Switch', category: 'IT', department: 'IT' }),
    countedQty: 0,
  },
];
...
describe('filterSortAssetCounts', () => {
  ...
  it('filters counted vs uncounted by countedQty', () => {
    expect(
      filterSortAssetCounts(rows, { ...base, filter: 'counted' }).map((r) => r.asset.id),
    ).toEqual(['a2']);
    expect(
  ...
  it('searches code/name/category/department case-insensitively', () => {
    expect(
      filterSortAssetCounts(rows, { ...base, search: 'finance' }).map((r) => r.asset.id),
    ).toEqual(['a2']);
    expect(
      filterSortAssetCounts(rows, { ...base, search: 'switch' }).map((r) => r.asset.id),
    ).toEqual(['a3']);
  });
});
